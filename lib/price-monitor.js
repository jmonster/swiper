...
console.log('sha: ' , luaSha);

redis.script('exists', luaSha, function(err, res) {
  if(!res[0]) {
    console.log('script doesnt exist...loading..');
...
function createDeal(deal) {
  ...
  Request.post({
    ...
    if (error) {
      return console.error('Failed to RETRIEVE product from: ',ENDPOINT, ' due to ', error);
    }
    console.log(body);
  });
}
