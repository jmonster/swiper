...
function Scutter(recipe,options) {
  ...
  var isHealthCheck = options.isHealthCheck || false;
  if(isHealthCheck) {
    this.healthCheck = {
    ...
    setTimeout(healthCheckTimeout, this.healthCheck.timeout, scutter);
  }
...
function _finish(scutter) {
  return function() {
    if(scutter.healthCheck) {
      scutter.emit('done', scutter.healthCheck);
      return;
    }
    ...
    if(IS_HTTP_DB && batch.length > 0) {
      stasher(null,null,batch,function(error, response) {
        scutter.emit('done');
      });
      batch = [];
    } else {
...
Scutter.prototype.step = function step(url,next,CB) {
  ...
  if(scutter.healthCheck.productsFound >= scutter.healthCheck.checkCount) {  
    q.kill();
    return CB(); 
  }

  if(next.stash) { scutter.healthCheck.productsFound++; }
  ...
  if(url.indexOf('http') !== 0) {
    console.warn('Invalid url: ', url.yellow);
    return CB();
  }
    ...
    function dbStore(blob) {
      ...
      if(IS_HTTP_DB) {
        ...
        if(batch.length >= argv['batch-size']) {
          stasher(scutter.conf.namespace,url,batch,cb);
          batch = [];
        } else {
          cb();
        }
