...
function Scutter(recipe,options) {
  ...
  if(this.isHealthCheck) {
    this.healthCheck = {
      ...
      productsEmitted: 0,
      timeout: options.productFindTimeout || 10000, // max time health check can run is (checkCount * timeout) -- generally if we find a product, we find multiple
      isHealthy: true,
      productChecks: {
        successes: [],
        failures: []
    ...
    this.on('product', function(blob) {
      if(scutter.healthCheck && (scutter.healthCheck.productsEmitted < scutter.healthCheck.checkCount)) {
        var valid = true;
