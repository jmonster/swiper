...
function Scutter(recipe,q) {
  ...
  q.drain = function() {
    rollbar.shutdown();
    scutter.onComplete && scutter.onComplete();
  }
...
Scutter.prototype.step = function step(url,next,CB) {
  ...
  Request(url, function(error1, response1, html1) {
    if (error1) { rollbar.handleError(error1); throw error1; }
    ...
    if (next.before) {
      next.before(response1, html1, function (error2,html2) {
        if (error2) { rollbar.handleError(error2); }
    ...
    function processHtml(html) {
      var select_iterator = function() {
        ...
        try      { var resolved = URL.resolve(url, el.attr('href')); }
          ...
          rollbar.handleErrorWithPayloadData(e, {'custom': {
            'href': el.attr('href'),
            'url': url
          }});
  ...
  function stash(url,html,next,cb) {
    ...
    if (beforeStash) {
      beforeStash(html,blob,function(error,blob2) {
        if (error) { rollbar.handleError(error); }
        stasher(stash,url,blob2,cb);
      });
    } else {
