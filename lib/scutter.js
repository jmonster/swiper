...
function Scutter(recipe,options) {
  ...
  db.Merchant.findOne({ homepage_url: { $regex: '(?i)' + scutter.conf.namespace } }, function(error, merchant) {
    if(error) { return console.warn('Could not find merchant id for recipe: '.red, error); }

    if(merchant) {
      scutter.conf.merchantId = merchant ? merchant.id : null;
    } else {
      console.warn('Could not find merchant id for recipe: '.red, scutter.conf.namespace);
    }
  });
      ...
      function _collect(cb) {
        collect(html,next,function(error2,hrefs) {
          ...
          hrefs.forEach(function(href) {
            ...
            if(next.stash) {
              href = normalizeUrl(href, scutter.conf.minimalQueryString);
            }
             ...
             */
            if(argv['unshift-queue'] || scutter.isHealthCheck) {
              q.unshift({
                href:href,
                next:next
              });
  ...
  function stash(url,html,next,cb) {
    ...
    if (stash.extract) {
      ...
      blob = {
        ...
        merchant: scutter.conf.merchantId
      };
...
function normalizeUrl(url, minimalQueryString) {
  if(!minimalQueryString) {
    minimalQueryString = findMinimalQueryString(url);
...
var hostToMinimalQueryString = {};
_.each(fs.readdirSync(Path.join(__dirname, '../recipes')), function(recipeName) {
  var recipe = require(Path.join(__dirname, '../recipes/', recipeName));
  var root = recipe.conf.root;
  var mqs = recipe.conf.minimalQueryString;

  var hostname = URL.parse(root).hostname;
  hostToMinimalQueryString[hostname] = mqs;
});

function findMinimalQueryString(url) {
  try {
    var parsedUrl = URL.parse(url);
    return hostToMinimalQueryString[parsedUrl.hostname];
  } catch(e) {
    console.warn('findMinimalQueryString: Could not parse url : ', url, e);
    return null;
  }
}
