...
server.route({
  method:  'GET',
  path:    '/public/{param*}',
  handler: { directory: { path: Path.resolve(__dirname,'./health') } }
});
...
handlebars.registerHelper('health-status', function(context) {
  ...
  if(isHealthy) {
    ...
    if(productsEmitted != checkCount
      && productsEmitted > 0
      && productsEmitted == successfulProducts) {
      return 'timeout';
    }
...
server.views({
  ...
  path: Path.resolve(__dirname,'./health/templates')
});
...
function recipeCheck(recipe) {
  return function(cb) {
    ...
    var scutter = new Scutter(recipe, { isHealthCheck: true, checkCount: 5 });
