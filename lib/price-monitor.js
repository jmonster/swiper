...
function handlePricePoint(message) {
  ...
  redis.evalsha(luaSha, 5, pricePointKey, yearAgo, now, DEAL_THRESHOLD, 'swiper-deals', function(error, dealResponse) {
    ...
    var isDeal = dealResponse[0];

    if(!isDeal) {
      return;
    }

    var averagePrice = dealResponse[1];
    var currentPrice = dealResponse[2];
    var previousPrice = dealResponse[3];
