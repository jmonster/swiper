...
function Scutter(recipe,q) {
  ...
  // pass `drain` through to our queue
...
Scutter.prototype.step = function step(url,next,CB) {
  ...
  Request(url, function(error1, response1, html1) {
    ...
    if (next.before) {
      recipe[next.before](response1,html1,function(error2,html2) {
  ...
  function stash(url,html,next,cb) {
    ...
    if (beforeStash) {
      beforeStash(html,blob,function(error,blob2) { stasher(stash,url,blob2,cb); });
    } else {
